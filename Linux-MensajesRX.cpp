...
int main(int argc, char *argv[])
{
    ...
    // mqd_t mq_open(const char *name, int oflag);
    mqDescriptor = mq_open("/miColaMen", O_RDONLY, NULL);
    if (mqDescriptor == -1)
    {
    ...
    numeroCaracteres = mq_receive(mqDescriptor, mensajebufer, 8192, NULL);

    printf("\e[0;33mMensaje recibido: %s\e[0m \n", mensajebufer);
    printf("\e[0;33mNúmero de caracteres recibido: %d\e[0m \n", numeroCaracteres);
