/*
    José Juan Ojeda Granados, 24-12-2021
    Programar "ls -l | wc > salida.txt"

    https://www.venea.net/man/es/dup(2)
    https://www.delftstack.com/es/howto/c/dup2-in-c/
*/
#include <iostream>
#include <sys/types.h>
#include <sys/wait.h>
...
int main(int argc, char *argv[])
{
    ...
    /*
    Condición que identifica al proceso del hijo generado por el primer fork,
    se encargara de ejecutar el programa del sistema "ls -la" y enviar su salida a
    ...
    */
    {
        ...
        //************************************
        close(fd1[WRITE_END]);
        // dup 2: Duplica FD (WRITE_END) a FD2 (STDOUT_FILENO), cerrando FD2 y haciéndolo
        // abierto en el mismo archivo.
        dup2(fd1[WRITE_END], (STDOUT_FILENO));
        //************************************
        /*
        A partir del dup2, todas las instrucciones que envíen datos a la stdout
        se envían al fd1[WITE_END] hasta la ejecución deĺ execlp, después del mismo
        se pierden ¿?
        De igual donde esté el close de
        */
        printf("FILE_NAME fd = %d\n", fd2);
        close(fd1[WRITE_END]);
        printf(" Cerrado: fd1[WRITE_END] \e[0;37mID %d\e[0m del proceso ID:\e[0;37mID %d\e[0m\n", fd1[WRITE_END], getpid());

        //************************************
        printf("Cerrado: fd1[WRITE_END] \e[0;37mID %d\e[0m del proceso ID:\e[0;37mID %d\e[0m\n", fd1[WRITE_END], getpid());
        execlp("/bin/ls", "ls", "-la", NULL);

        fd2 = open(FILE_NAME, O_WRONLY | O_APPEND);
